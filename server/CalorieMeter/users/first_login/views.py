...
def form(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if h <= 0 or w <= 0:
                errors.append("身長・体重は0より大きい値を入力してください。")
        ...
        if target_weight_raw not in (None, "", " "):
            try:
                ...
                if t <= 0:
                    errors.append("目標体重は0より大きい値を入力してください。")
            except (TypeError, ValueError):
                errors.append("目標体重は数値で入力してください。")
        ...
        profile.save(update_fields=["display_name","avatar", "bio", "height_cm", "weight_kg", "target_weight"])
